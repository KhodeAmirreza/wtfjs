...
> **&#x202B;⚠️ توجه:** اگر از خوندن این رفرنس لذت میبرید، لطفا [حمایت از نویسنده‌ش رو در نظر بگیرید](#-supporting).
...
```
### 💡 توضیح:
عملگر == توی جاوا اسکریپت مقدار هر دو طرف رو تبدیل میکنه به عدد تا اونها رو مقایسه کنه، توی این مثال هر دو طرف تبدیل به `0` میشن به دلایل مختلفی. آرایه ها مقدار های truthy هستن پس سمت راست مخالف یه مقدار truthy میشه `false‍‍` که 0 هم یک مقدار falsy هست. همچنین در سمت چپ یه آرایه ی خالی هنگام تبدیل شدن به عدد، میشه صفر بدون اینکه تبدیل به boolean بشه و آرایه های خالی تبدیل به `0` میشن با وجود truthy بودن.
نحوه‌ی ساده شدن این عبارت:
```js
+[] == +![];
0 == +false;
0 == 0;
...
همچنین ببینید: [`[]` is truthy, but not `true`](#-is-truthy-but-not-true)
